...
def Mol(E,A,B,n):
    ...
    An = [[]]
    for i in range(n-1):
        Ans = input('¿La %i-esima molecula es orto o meta(o/m)?'%(i))
        case Ans:
            An[0].append(Amol)
...
def main():
    ...
    #Valores de la energía de citio y los hopings
    A = 0
    B = -0.5
    ...
    while True:
        ...
        ax.plot(E,T,label=nom)
        Ans = input("¿Parar el programa e imprimir?(s/n): ")
        if Ans == 's':
            break
